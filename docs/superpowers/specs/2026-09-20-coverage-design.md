...
- **Integration** (`src/app/dashboard/coverage/actions.integration.test.ts`): admin loads view, adds and removes presence, whole-night adds one row per slot; captain and anonymous get `{ status: false, message: "Unauthorized." }`; past date rejected; non-slot time rejected; `sendCoverageDigestForDate` second call is a no-op (dedupe).
