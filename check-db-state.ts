...
async function checkDatabaseState() {
    ...
    try {
        ...
        seasonsColumns.rows.forEach(row => console.log('  -', row.column_name))
        
        // Check if season_events exist and count
        const eventsCount = await client.query('SELECT COUNT(*) as count FROM season_events')
        ...
        if (currentSeason.rows.length > 0) {
            ...
            if (seasonEvents.rows.length === 0) {
                ...
                seasonEvents.rows.forEach(row => {
                    console.log(`  ${row.event_type}: ${row.event_date} (${row.count} entries)`)
                })
            }
        }
