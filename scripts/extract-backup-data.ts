...
async function extractAndMigrateData() {
    ...
    try {
        ...
        const stringsOutput = execSync(`/usr/bin/strings "${BACKUP_FILE}"`, {
            encoding: "utf8",
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (
                line.includes("COPY public.signups") &&
                line.includes("dates_missing")
            ) {
            ...
            if (
                inSignupsSection &&
                (line.startsWith("\\") || line.includes("public."))
            ) {
                break
            }
        ...
        console.log(
            `🔍 Found ${signupDataLines.length} potential signup records`
        )

        if (signupDataLines.length === 0) {
            console.log(
                "❌ No signup data found in backup - trying alternative parsing"
            )
        ...
        for (const line of signupDataLines.slice(0, 20)) {
        ...
        console.log(
            `✅ Parsed ${signupsWithDates.length} signups with availability data`
        )
        ...
        if (signupsWithDates.length > 0) {
            ...
            signupsWithDates.slice(0, 5).forEach((signup) => {
                console.log(
                    `  Signup ${signup.signupId} (Season ${signup.seasonId}):`
                )
            ...
            console.log(
                "❌ No signup availability data could be parsed from backup"
            )
        }
    } catch (error) {
...
// biome-ignore lint/suspicious/noExplicitAny: one-off migration script
async function migrateAvailabilityData(signupsData: any[]) {
    ...
    try {
        ...
        console.log(
            `📅 Found ${eventsResult.rows.length} events to match against`
        )
                ...
                if (matchingEvent) {
                    ...
                    try {
                        ...
                        if (!message.includes("foreign key")) {
                            console.log(
                                `⚠️  Error inserting availability for signup ${signup.signupId}: ${message}`
                            )
                        }
                    }
                }
        ...
        console.log(
            `✅ Successfully migrated ${migratedCount} availability records`
        )

        // Verify the migration
        const finalCount = await client.query(
            "SELECT COUNT(*) as count FROM player_unavailability"
        )
        console.log(
            `📊 Total player unavailability records: ${finalCount.rows[0].count}`
        )
    } catch (error) {
