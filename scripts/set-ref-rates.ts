...
async function main() {
    ...
    const seasons = await db.execute(sql`SELECT id, code, year, season, certified_ref_rate, uncertified_ref_rate FROM seasons ORDER BY id DESC LIMIT 5`)
    const rows = (seasons as any).rows ?? seasons
    console.log('Current seasons:')
    for (const row of rows as any[]) {
        console.log(`  id=${row.id} code="${row.code}" year=${row.year} season=${row.season} certified=${row.certified_ref_rate} uncertified=${row.uncertified_ref_rate}`)
    }
