/**
 * Usage:
 *   echo -e "alice@example.com\nbob@example.com" | npx tsx scripts/set-notification-list.ts <status>
 *   cat emails.txt | npx tsx scripts/set-notification-list.ts <status>
 *
 * Sets the notification_list field to <status> for all users whose email matches
...
async function main() {
    ...
    if (!status) {
        console.error(
            "Usage: cat emails.txt | npx tsx scripts/set-notification-list.ts <status>"
        )
        process.exit(1)
    }
    ...
    console.log(
        `Updating notification_list to "${status}" for ${matched.length} user(s):`
    )
    ...
    await db
        .update(users)
        .set({ notification_list: status, updatedAt: new Date() })
        .where(
            inArray(
                users.id,
                matched.map((r) => r.id)
