...
export async function GET() {
    ...
    try {
        ...
        if (config.phase !== "prep_tryout_week_3") {
            return NextResponse.json(
                {
                    error: "Week 3 tryout sheets are only available during Prepare for Tryout Week 3."
                },
                { status: 403 }
            )
        }
        ...
        if (!week3RosterRow) {
            return NextResponse.json(
                {
                    error: "Week 3 tryout sheets are unavailable until week 3 roster records exist for this season."
                },
                { status: 404 }
            )
        }
        ...
        const rosterRows = await db
            ...
            .leftJoin(
                signups,
                and(
                    eq(signups.player, week3Rosters.user),
                    eq(signups.season, config.seasonId)
                )
            )
            .where(eq(week3Rosters.season, config.seasonId))
            .orderBy(divisions.level, week3Rosters.team_number, users.last_name)

        if (rosterRows.length === 0) {
            return NextResponse.json(
                {
                    error: "No week 3 tryout roster rows found for current season."
                },
                { status: 404 }
            )
        }
        ...
        for (const sessionNumber of sessions) {
            for (
                ...
                const title = `${capitalize(config.seasonName)} ${config.seasonYear} Week 3 Tryout Sheets - Session ${sessionNumber} (${sessionTime}) - Page ${pageIndex + 1}`
        ...
        const seasonSlug = config.seasonName
            ...
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "")
        const downloadFileName = `bsd-week3-${seasonSlug}-${config.seasonYear}.pdf`
        ...
        if (session?.user) {
            await logAuditEntry({
                ...
                summary: `Downloaded week 3 tryout sheets PDF for season ${config.seasonId}`
            })
        }
        ...
        console.error("Error creating week 3 tryout sheets PDF:", error)
        return NextResponse.json(
            { error: "Failed to generate week 3 tryout sheets PDF." },
            { status: 500 }
        )
    }
}
