...
import { notificationOptouts, week3Rosters } from "@/database/schema"
...
describe("updateWeek3Rosters", () => {
    ...
    it("does not enforce captain-division locks (week 3 captains are editable)", async () => {
        const season = await createSeason()
    ...
    it("replaces the season's rosters with the submitted slots", async () => {
        ...
        const rows = await db
            .select()
            .from(week3Rosters)
            .where(eq(week3Rosters.season, season.id))
...
describe("sendWeek3RosterNotifications", () => {
    beforeEach(() => {
    ...
    it("requires admin access", async () => {
        await createUserWithRoles([{ role: "captain" }])
        const result = await sendWeek3RosterNotifications(
            [
                {
                    userId: "x",
                    divisionId: 1,
