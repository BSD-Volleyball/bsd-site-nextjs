...
import { divisions, users, week3Rosters } from "@/database/schema"
...
export async function GET() {
    ...
    try {
        ...
        for (const raw of rawRows) {
            ...
            if (existing) {
                existing.courtEntries.push({ courtNumber, sessionNumber })
                if (sessionNumber < existing.primarySessionNumber) {
                    existing.primarySessionNumber = sessionNumber
                }
            } else {
                userMap.set(raw.userId, {
                    primarySessionNumber: sessionNumber,
                    courtEntries: [{ courtNumber, sessionNumber }],
                    oldId: raw.oldId,
        ...
        const normalizedRows = Array.from(userMap.values()).sort((a, b) => {
        ...
        if (normalizedRows.length === 0) {
            return NextResponse.json(
                {
                    error: "No week 3 roster rows found."
                },
                { status: 404 }
            )
        }
        ...
        const seasonSlug = config.seasonName
            ...
            .replace(/\s+/g, "-")
            .replace(/[^a-z0-9-]/g, "")
        const downloadFileName = `bsd-week3-nametags-${seasonSlug}-${config.seasonYear}.pdf`
        ...
        if (session?.user) {
            await logAuditEntry({
                ...
                summary: `Downloaded week 3 nametag labels PDF for season ${config.seasonId}`
            })
        }
        ...
        console.error("Error creating week 3 nametag labels PDF:", error)
        return NextResponse.json(
