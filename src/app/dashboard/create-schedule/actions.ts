...
export const writeRegularSeasonSchedule = withAction(
    async (seasonId: number): Promise<ActionResult> => {
        const hasAccess = await isAdminOrDirectorBySession()
        if (!hasAccess) {
        ...
        try {
            ...
            for (const div of data.divisions) {
                if (div.teams.length !== div.teamCount) {
                    return fail(
                        `${div.divisionName} expects ${div.teamCount} teams but has ${div.teams.length}. Please create all teams first.`
                    )
                }
            }
            ...
            if (allMatches.length === 0) {
                return fail("No matches generated. Check team data.")
            }
...
export const writePlayoffSchedule = withAction(
    async (seasonId: number): Promise<ActionResult> => {
        const hasAccess = await isAdminOrDirectorBySession()
        if (!hasAccess) {
            return fail("You don't have permission to perform this action.")
