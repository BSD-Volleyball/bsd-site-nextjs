...
/**
 * Builds the tryout-sheets PDF response for tryout week 2 or 3. The two
 * weeks share identical logic; only the roster table, phase gate, tryout
    ...
    try {
        ...
        const rosterRows = await db
            ...
            .leftJoin(
                signups,
                and(
                    eq(signups.player, rosterTable.user),
        ...
        const sessionTimeMap: Record<1 | 2 | 3, string> = {
            1: formatEventTime(weekSlots[0]?.startTime ?? ""),
            2: formatEventTime(weekSlots[1]?.startTime ?? ""),
        ...
        for (const sessionNumber of sessions) {
            for (
                ...
                const title = `${capitalize(config.seasonName)} ${config.seasonYear} Week ${week} Tryout Sheets - Session ${sessionNumber} (${sessionTime}) - Page ${pageIndex + 1}`
