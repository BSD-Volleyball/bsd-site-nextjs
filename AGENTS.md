...
- Slot jobs: `slotTaskGroups(day, slot)` in `src/lib/coverage/tasks.ts` is the one source of the gym task text for both the page (collapsed `<details>` under each slot) and the digest. Setup goes to the first timed slot, Mid-way and Cleanup to the last (a single-slot night gets all three; the TBD slot gets none). Lines switch on `eventType` (regular vs playoff clipboard/flag handling) and on `CoverageDate.nextEventType` (playoff scoresheets when playoffs are next; flags in the bins on the last regular-season week; no scoresheet pull when the season is over). `slotAssigneeNames(slot)` = the people who count in that slot, shown as who the jobs fall to.
