...
- Machine-readability affordances, all deliberate: four corner fiducials (bottom-right is half-size, so a photo's orientation is unambiguous), a QR code encoding `/dashboard/enter-scores?date=&court=&v=`, heavy empty two-digit FINAL boxes per team per game, checkboxes for timeouts/forfeit/captain initials, and a tiny `#<matchId>` per block. Bump `TEMPLATE_VERSION` in `sheet-config.ts` whenever the geometry changes in a way that would break a reader built against the old layout.
