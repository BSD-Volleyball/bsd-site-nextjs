...
async function createMinimalSeasonEvents() {
    ...
    try {
        ...
        await client.query("DELETE FROM season_events WHERE season_id = $1", [
            seasonId
        ])
        ...
        const events = [
            ...
            // Drafts (6 divisions)
            {
                ...
                date: "2026-03-15",
                order: 1,
                label: "AA Division Draft"
            },
            {
                type: "draft",
                date: "2026-03-15",
                order: 2,
                label: "A Division Draft"
            },
            {
                type: "draft",
                date: "2026-03-16",
                order: 3,
                label: "BB Division Draft"
            },
            {
                type: "draft",
                date: "2026-03-16",
                order: 4,
                label: "B Division Draft"
            },
            {
                type: "draft",
                date: "2026-03-17",
                order: 5,
                label: "CC Division Draft"
            },
            {
                type: "draft",
                date: "2026-03-17",
                order: 6,
                label: "C Division Draft"
            },
            ...
            // Late registration cutoff
            {
                ...
                date: "2026-02-01",
                order: 1,
                label: "Late Registration Begins"
            }
        ]
        ...
        for (const event of events) {
            ...
            console.log(
                `Created \${event.type} event: \${event.label} on \${event.date}`
            )
        ...
        const _eventCount = await client.query(
            "SELECT COUNT(*) as count FROM season_events WHERE season_id = $1",
            [seasonId]
        )
        const _slotCount = await client.query(
            `
        ...
        console.log(`\nNext steps:`)
        console.log(
            `1. Visit /dashboard/season-config/ to review and adjust dates/times`
        )
        console.log(
            `2. The dates above are placeholders - please update them to match your actual schedule`
        )
    } catch (error) {
